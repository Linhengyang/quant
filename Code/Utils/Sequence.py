...
def strided_slice_1darr(
    ...
    Stride len/stepsize = stride
    '''

    assert len(arr) >= window_size,\
        '1-D array length must be larger or equal to window_size'
...
def strided_slicing_w_residual(
        length:int,
        window_size:int,
        stride:int
    ...
    # strided_slices[-1][0] 开始到 strided_slices[-1][-1],
    # 长度是window_size, 但是 strided_slices[-1][-1]到末尾，不足window_size

    residual_range = range(strided_slices[-1][-1]+1, length) # 从strided_slices[-1][-1] + 1 到末尾, 可为空

    # 从strided_slices[-1][0] + stride 到末尾, 可为空
    last_short_range = range(strided_slices[-1][0] + stride, length)
...
def strided_indexing_w_residual(
        length:int,
        window_size:int,
        stride:int
